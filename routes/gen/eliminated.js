...
router.imageprocess = class eliminated extends ImageCode {
  ...
  async process(msg) {
    ...
    let fire = await Jimp.read(path.join(Constants.assetPath, 'eliminatedFire.png'))
    let img = im(864, 1000).command('convert')
    img.font(path.join(Constants.assetPath, 'fonts', 'bignoodletoo.ttf'), 70)
    ...
    img2.font(path.join(Constants.assetPath, 'fonts', 'bignoodletoo.ttf'), 70)
    ...
    img3.font(path.join(Constants.assetPath, 'fonts', 'bignoodletoo.ttf'), 70)
